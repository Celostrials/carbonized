...
import { MockCarbon, CarbonizedCollection, CarbonRewards, MockNFT, MockReward } from "../types"
import { SignerWithAddress } from "@nomiclabs/hardhat-ethers/signers"

chai.use(solidity)

describe("Carbonized Rewards Test", function () {
  ...
  this.beforeEach(async function () {
    ...
    // Deploy mock reward
    const mockRewardFactory = await ethers.getContractFactory("MockReward")
    mockReward = (await mockRewardFactory.deploy(stringToEth("100000"))) as MockReward
    // Deploy CarbonRewards Contract
    const carbonRewardsFactory = await ethers.getContractFactory("CarbonRewards")
    let args = [accounts[0].address, mockReward.address]
    ...
    // set approval for carbonRewards to transfer mock reward
    await expect(
      mockReward.approve(carbonRewards.address, ethers.constants.MaxUint256)
    ).to.not.be.reverted
  ...
  it("Claim carbon rewards", async function () {
    ...
    // start reward period with 100 rewards
  ...
  it("Claim reward balances", async function () {
    ...
    await expect(carbonizedCollection.transferFrom(accounts[0].address, accounts[1].address, 3)).to
      .not.be.reverted
    // start reward period for 100 rewards
    await expect(carbonRewards.notifyRewardAmount(stringToEth("100.0"))).to.not.be.reverted
    // check rewards balance of 1
    expect(ethToString(await mockReward.balanceOf(accounts[1].address))).to.equal("0.0")
    // check rewards of 1
    expect(ethToString(await carbonRewards.rewards(accounts[1].address))).to.equal("0.0")
    // increase time by 10 seconds
    await ethers.provider.send("evm_increaseTime", [10])
    ...
    // check rewards balance before claim rewards
    expect(ethToString(await mockReward.balanceOf(accounts[1].address))).to.equal("0.0")
    // claim rewards
    await expect(carbonRewards.connect(accounts[1]).getReward()).to.not.be.reverted
    // check rewards balance after claim
    expect(ethToString(await mockReward.balanceOf(accounts[1].address))).to.equal("100.0")
  })
